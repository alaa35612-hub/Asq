    ...
    *,
    ...
    for symbol in ranked:
        if len(results) >= config.max_symbols:
            break
        if config.pump_change_threshold > 0 and config.pump_timeframe:
            if symbol not in pump_cache:
                pump_cache[symbol] = _fetch_percent_change(
                    exchange,
                    symbol,
                    config.pump_timeframe,
                    candles=2,
                )
            change = pump_cache.get(symbol)
            if change is not None and change >= config.pump_change_threshold:
                continue
