...
@dataclass(frozen=True)
class _PumpFilterDefaults:
    change_threshold: float = 0.0
    timeframe: str = ""
    candles: int = 2


PUMP_FILTER_DEFAULTS = _PumpFilterDefaults()


class PumpFilter:
    """Stateful helper that caches pump-percentage lookups per symbol."""

    __slots__ = ("_exchange", "threshold", "timeframe", "candles", "_cache")

    def __init__(
        self,
        exchange: Optional["ccxt.binanceusdm"],
        *,
        threshold: float,
        timeframe: str,
        candles: int = 2,
        ...
        self.timeframe = str(timeframe or "").strip()
        self.candles = max(2, int(candles))
    ...
    def percent_change(self, symbol: str) -> Optional[float]:
        if not self.enabled:
            return None
        key = str(symbol or "").upper()
        if key not in self._cache:
            self._cache[key] = _fetch_percent_change(
                self._exchange,
                key,
                self.timeframe,
    ...
    def should_skip(self, symbol: str) -> bool:
        change = self.percent_change(symbol)
...
def print_symbol_summary(index: int, symbol: str, timeframe: str, candle_count: int, metrics: Dict[str, Any]) -> None:
    ...
    change_value = metrics.get("pump_change_percent")
    if isinstance(change_value, (int, float)):
        timeframe_hint = metrics.get("pump_change_timeframe")
        label = "نسبة الارتفاع"
        if isinstance(timeframe_hint, str) and timeframe_hint.strip():
            label = f"{label} ({timeframe_hint.strip()})"
        header_lines.append(f"{ANSI_DIM}{label}: {change_value:+.2f}%{ANSI_RESET}")
    header = "\n".join(header_lines)
...
def scan_binance(
    ...
    *,
    ...
    pump_timeframe = str(pump_timeframe or "").strip()
    pump_filter: Optional[PumpFilter] = None
    if pump_change_threshold > 0 and pump_timeframe:
        pump_filter = PumpFilter(
            exchange,
            threshold=pump_change_threshold,
            timeframe=pump_timeframe,
            candles=PUMP_FILTER_DEFAULTS.candles,
        )
    ...
    for idx, symbol in enumerate(all_symbols):
        pump_change: Optional[float] = None
        if pump_filter and pump_filter.enabled:
            pump_change = pump_filter.percent_change(symbol)
            if pump_change is not None and pump_change >= pump_filter.threshold:
                print(
                    f"تخطي {_format_symbol(symbol)} (نسبة الارتفاع {pump_change:.2f}% ≥ الحد {pump_filter.threshold:.2f}%)",
                    flush=True,
                )
                if tracer and tracer.enabled:
                    tracer.log(
                        "scan",
                        "symbol_skipped_pump_filter",
                        timestamp=None,
                        symbol=symbol,
                        change=pump_change,
                        threshold=pump_filter.threshold,
                        timeframe=pump_filter.timeframe,
                    )
                continue
        ...
        if pump_change is not None:
            metrics["pump_change_percent"] = pump_change
            metrics["pump_change_timeframe"] = pump_filter.timeframe if pump_filter else pump_timeframe
        latest_events = metrics.get("latest_events") or {}
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        "--pump-threshold",
        type=float,
        default=PUMP_FILTER_DEFAULTS.change_threshold,
        help="تخطي العملات التي ارتفعت على الأقل بهذه النسبة خلال الإطار الزمني المحدد",
    )
    parser.add_argument(
        "--pump-timeframe",
        type=str,
        default=PUMP_FILTER_DEFAULTS.timeframe or EDITOR_AUTORUN_DEFAULTS.timeframe,
        help="الإطار الزمني المستخدم لحساب نسبة الارتفاع",
    )
    parser.add_argument(
        "--pullback-report",
        action="store_true",
    ...
    if args.pump_threshold < 0.0:
        parser.error("--pump-threshold يجب أن يكون رقمًا غير سالب")
    pump_timeframe = str(args.pump_timeframe or "").strip()
    if args.pump_threshold > 0.0 and not pump_timeframe:
        parser.error("يجب تحديد --pump-timeframe عند استخدام --pump-threshold")
    args.pump_timeframe = pump_timeframe
    ...
    runtime, summaries = scan_binance(
        ...
        args.concurrency,
        tracer,
        pump_change_threshold=float(args.pump_threshold),
...
# ============================================================================
# Binance USDT-M scanner with pump filter
# ============================================================================

def _pct_24h(ticker: Dict[str, Any]) -> float:
    value = ticker.get("percentage")
    info = ticker.get("info") if isinstance(ticker, dict) else None
    if value is None and isinstance(info, dict):
        value = info.get("priceChangePercent")
    try:
        return float(value)
    except Exception:
        return 0.0
...
def _qv_24h(ticker: Dict[str, Any]) -> float:
    value = ticker.get("quoteVolume")
    info = ticker.get("info") if isinstance(ticker, dict) else None
    if value is None and isinstance(info, dict):
        value = info.get("quoteVolume")
    try:
...
def _fetch_percent_change(exchange: "ccxt.binanceusdm", symbol: str, timeframe: str, *, candles: int = 2) -> Optional[float]:
    if candles < 2:
        candles = 2
    try:
...
def _detect_market_universe(exchange: "ccxt.binanceusdm") -> List[str]:
    markets = exchange.load_markets()
    return [
        symbol
        for symbol, meta in markets.items()
        if meta.get("linear") and meta.get("quote") == "USDT" and meta.get("type") == "swap"
    ]
...
def _pick_symbols(
    ...
    *,
    ...
    pump = pump_filter
    if pump is None and config.pump_change_threshold > 0 and config.pump_timeframe:
        pump = PumpFilter(
            exchange,
            threshold=config.pump_change_threshold,
            timeframe=config.pump_timeframe,
            candles=PUMP_FILTER_DEFAULTS.candles,
        )
    ...
    try:
        ...
        tickers = {}
    ranked = sorted(universe, key=lambda sym: _pct_24h(tickers.get(sym) or {}), reverse=True)
    results: List[str] = []
    for symbol in ranked:
        if len(results) >= config.max_symbols:
            break
        if pump and pump.enabled and pump.should_skip(symbol):
            continue
        results.append(symbol)
    return results


def _parse_args_scanner() -> Tuple[_ScannerConfig, argparse.Namespace]:
    parser = argparse.ArgumentParser(prog="SMC Binance Scanner")
    parser.add_argument("--symbol", "-s", default="")
    parser.add_argument("--timeframe", "-t", default=EDITOR_AUTORUN_DEFAULTS.timeframe)
    parser.add_argument("--limit", "-l", type=int, default=EDITOR_AUTORUN_DEFAULTS.candle_limit)
    parser.add_argument("--max-symbols", "-n", type=int, default=EDITOR_AUTORUN_DEFAULTS.max_symbols)
    parser.add_argument(
        "--pump-threshold",
        type=float,
        default=PUMP_FILTER_DEFAULTS.change_threshold,
    ...
    parser.add_argument(
        "--pump-timeframe",
        default=PUMP_FILTER_DEFAULTS.timeframe or EDITOR_AUTORUN_DEFAULTS.timeframe,
        help="الإطار الزمني المستخدم لفحص نسبة الارتفاع",
    )
    parser.add_argument("--recent", type=int, default=EDITOR_AUTORUN_DEFAULTS.recent_bars)
    parser.add_argument("--verbose", "-v", action="store_true", default=False)
    args, _ = parser.parse_known_args()
    if args.pump_threshold < 0:
        parser.error("--pump-threshold يجب أن يكون رقمًا غير سالب")
    if args.pump_threshold > 0 and not str(args.pump_timeframe or "").strip():
        parser.error("يجب تحديد --pump-timeframe عند استخدام --pump-threshold")
    config = _ScannerConfig(
...
def _prepare_indicator_inputs() -> "IndicatorInputs":
    pullback = PullbackInputs(showHL=False, showMn=False)
    structure = MarketStructureInputs(showSMC=True, lengSMC=40, showCircleHL=True)
    order_flow = OrderFlowInputs(showISOB=True, showMajoinMiner=False)
    fvg = FVGInputs(show_fvg=True)
    liquidity = LiquidityInputs(currentTF=True, displayLimit=20)
    demand_supply = DemandSupplyInputs(mittigation_filt="CLOSE")
    order_block = OrderBlockInputs(poi_type="Mother Bar")
    structure_util = StructureInputs(isOTE=True, markX=True)
    ict = ICTMarketStructureInputs(swingSize=10)
...
def _android_cli_entry() -> int:
    if ccxt is None:
        ...
        return 2
    config, args = _parse_args_scanner()
    inputs = _prepare_indicator_inputs()
    try:
        SmartMoneyAlgoProE5
    ...
    pump_filter: Optional[PumpFilter] = None
    if config.pump_change_threshold > 0 and config.pump_timeframe:
        pump_filter = PumpFilter(
            exchange,
            threshold=config.pump_change_threshold,
            timeframe=config.pump_timeframe,
    ...
    if not symbols:
        ...
        return 4
    recent_window = config.recent_bars
    alerts_total = 0
    if hasattr(inputs, "console"):
        try:
    ...
    for index, symbol in enumerate(symbols, 1):
        ...
        if pump_filter and pump_filter.enabled:
            ...
            if pump_change is not None:
                metrics["pump_change_percent"] = pump_change
                metrics["pump_change_timeframe"] = pump_filter.timeframe
        latest_events = metrics.get("latest_events") or {}
        ...
        if not recent_hits:
            if args.verbose:
                print(f"[{index}/{len(symbols)}] تخطي {symbol} لعدم وجود إشارات حديثة")
            continue
        recent_alerts = list(getattr(runtime, "alerts", []))
        if recent_alerts and hasattr(runtime, "series") and runtime.series.length() > 0:
            try:
                ...
                if cutoff_time:
                    recent_alerts = [
                        (ts, title)
                        for ts, title in recent_alerts
                        if ts >= cutoff_time
                    ]
            except Exception:
                pass
        if recent_alerts or args.verbose:
            _print_ar_report(symbol, config.timeframe, runtime, exchange, recent_alerts)
            alerts_total += len(recent_alerts)
    if args.verbose:
        print(f"\nاكتملت عملية المسح. عدد التنبيهات: {alerts_total}")
    return 0


# === Auto-args when launched from editor (no CLI flags) ====== FUTURES-ONLY ==
def __editor_autoargs__() -> None:
    if len(sys.argv) == 1:
        ...
        sys.argv += [
            ...
            "--max-symbols",
            str(defaults.max_symbols),
            "--recent",
            str(defaults.recent_bars),
        ]


def __router_main__() -> int:
    __editor_autoargs__()
